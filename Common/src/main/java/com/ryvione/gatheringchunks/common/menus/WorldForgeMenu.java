...
public class WorldForgeMenu extends BaseInventoryContainerMenu {
    ...
    public WorldForgeMenu(int menuId, Inventory inventory, Container container, ContainerData containerData) {
        ...
        addSlot(new Slot(container, WorldForgeBlockEntity.SLOT_INPUT, 58, 35) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return blockContainer.canPlaceItem(WorldForgeBlockEntity.SLOT_INPUT, stack);
            }
        });
    ...
    protected boolean quickMoveToContainer(ItemStack stack) {
        if (this.blockContainer.canPlaceItem(WorldForgeBlockEntity.SLOT_INPUT, stack)) {
